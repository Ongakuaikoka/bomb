...
class Input:
    ...
    def __init__(self, **options):
        ...
        self.options = Config(options, ['x', '0'], ['y', '0'], ['font', 'pygame.font.SysFont("a_FuturaRound", 32)'],
    ...
    def get_size(self):
	self.size = (self.font.size(str(self.value))[0] + self.font.size(str(self.prompt))[0], self.font.size(str(self.value))[1])
	return self.size
	...
	pressed = pygame.key.get_pressed()#Add ability to hold down delete key and delete text
        if self.pause == 10 and pressed[K_BACKSPACE]:
            self.pause = 0
		...
		elif event.key in (K_RETURN, K_KP_ENTER): return self.value
                if not self.shifted:
